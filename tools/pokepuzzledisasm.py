...
def read_symfile(path='pokepuzzle.sym'):
	"""
...
class Disassembler(object):
	...
	def output_bank_opcodes(self, start_offset, stop_offset, hard_stop=False, parse_data=False, recursive=False):
		if not recursive:
			return self._output_bank_opcodes(start_offset, stop_offset, hard_stop=hard_stop, parse_data=parse_data)[0]
		
		processed_routines = {}
		self.queue.append(start_offset)
		while len(self.queue) != 0:
			next_addr = self.queue.pop()
			if next_addr not in processed_routines:
				processed_routines[next_addr] = self._output_bank_opcodes(next_addr, stop_offset, hard_stop=hard_stop, parse_data=parse_data)[0]

		addresses = [k for k in processed_routines.keys()]
		addresses = sorted(addresses)
		out = ""
		for addr in addresses:
			out += processed_routines[addr] + "\n\n"
		return out
	...
	def _output_bank_opcodes(self, start_offset, stop_offset, hard_stop=False, parse_data=False):
		...
		while True:
			...
			if not is_data or not parse_data:
				...
				# args for farcall
				farcall_arg_addr = rom[offset+3] + (rom[offset+4] << 8)
				farcall_arg_bank = rom[offset+5]

				# args for copy_data
				copy_data_arg_dest_addr = rom[offset+3] + (rom[offset+4] << 8)
				copy_data_arg_dest_bank = rom[offset+5]
				copy_data_arg_source_addr = rom[offset+6] + (rom[offset+7] << 8)
				copy_data_arg_source_bank = rom[offset+8]
				copy_data_arg_len  = rom[offset+9] + (rom[offset+10] << 8)
				copy_data_arg_interval  = rom[offset+11] + (rom[offset+12] << 8)
				copy_data_arg_iterations  = rom[offset+13] + (rom[offset+14] << 8)

				safe_copy_data_arg_len  = rom[offset+9]
				safe_copy_data_arg_interval  = rom[offset+10]
				safe_copy_data_arg_iterations  = rom[offset+11]

				# args for fill_mem
				fill_arg_1 = rom[offset+3]
				fill_arg_2 = rom[offset+4]
				fill_arg_3 = rom[offset+5] + (rom[offset+6] << 8)

				# args for decompress
				decompress_arg_addr = rom[offset+3] + (rom[offset+4] << 8)
				decompress_arg_bank = rom[offset+5]
				decompress_arg_3 = rom[offset+6]

				# args for func_621
				func_621_arg_6  = rom[offset+9]
				func_621_arg_len  = rom[offset+10] + (rom[offset+11] << 8)
				func_621_arg_interval  = rom[offset+12] + (rom[offset+13] << 8)
				func_621_arg_iterations  = rom[offset+14] + (rom[offset+15] << 8)
				...
				# opcodes with a pointer as an argument
					...
					if opcode_byte in call_commands + absolute_jumps:
						...
						# if this is a call or jump opcode and the target label is not defined, create an undocumented label descriptor
							...
							self.queue.append(target_offset)
				...
				# handle farcalls
				if opcode_byte == 0xcd:
					if target_label in ["Farcall", "Farcall_SaveRegs"]:
						farcall_label = self.find_label(farcall_arg_addr, farcall_arg_bank)
						farcall_macro = "farcall" if target_label == "Farcall" else "farcall_saveregs"
						if farcall_label is None:
							target_offset = get_global_address(farcall_arg_addr, farcall_arg_bank)
							self.queue.append(target_offset)
							opcode_output_str = f"{farcall_macro} ${farcall_arg_addr:04x}, ${farcall_arg_bank:02x} ; Func_{target_offset:0x}"
						else:
							opcode_output_str = f"{farcall_macro} {farcall_label}"
						offset += 3
					elif target_label == "CopyData":
						dest_label = self.find_label(copy_data_arg_dest_addr, copy_data_arg_dest_bank)
						source_label = self.find_label(copy_data_arg_source_addr, copy_data_arg_source_bank)
						if dest_label is None or source_label is None:
							dest_offset = get_global_address(copy_data_arg_dest_addr, copy_data_arg_dest_bank)
							source_offset = get_global_address(copy_data_arg_source_addr, copy_data_arg_source_bank)
							opcode_output_str = f"copy_data ${copy_data_arg_dest_addr:04x}, ${copy_data_arg_dest_bank:02x}, ${copy_data_arg_source_addr:04x}, ${copy_data_arg_source_bank:02x}, ${copy_data_arg_len:0x} ; {dest_offset:0x}, {source_offset:0x}"
						else:
							opcode_output_str = f"copy_data {dest_label}, {source_label}, ${copy_data_arg_len:0x}"
						offset += 8
					elif target_label == "Func_802":
						dest_label = self.find_label(copy_data_arg_dest_addr, copy_data_arg_dest_bank)
						source_label = self.find_label(copy_data_arg_source_addr, copy_data_arg_source_bank)
						if dest_label is None or source_label is None:
							dest_offset = get_global_address(copy_data_arg_dest_addr, copy_data_arg_dest_bank)
							source_offset = get_global_address(copy_data_arg_source_addr, copy_data_arg_source_bank)
							opcode_output_str = f"copy_box ${copy_data_arg_dest_addr:04x}, ${copy_data_arg_dest_bank:02x}, ${copy_data_arg_source_addr:04x}, ${copy_data_arg_source_bank:02x}, ${copy_data_arg_len:0x}, ${copy_data_arg_interval:0x}, ${copy_data_arg_iterations:0x} ; {dest_offset:0x}, {source_offset:0x}"
						else:
							opcode_output_str = f"copy_box {dest_label}, {source_label}, ${copy_data_arg_len:0x}, ${copy_data_arg_interval:0x}, ${copy_data_arg_iterations:0x}"
						offset += 12
					elif target_label == "SafeCopyData":
						dest_label = self.find_label(copy_data_arg_dest_addr, copy_data_arg_dest_bank)
						source_label = self.find_label(copy_data_arg_source_addr, copy_data_arg_source_bank)
						if dest_label is None or source_label is None:
							dest_offset = get_global_address(copy_data_arg_dest_addr, copy_data_arg_dest_bank)
							source_offset = get_global_address(copy_data_arg_source_addr, copy_data_arg_source_bank)
							opcode_output_str = f"safe_copy_data ${copy_data_arg_dest_addr:04x}, ${copy_data_arg_dest_bank:02x}, ${copy_data_arg_source_addr:04x}, ${copy_data_arg_source_bank:02x}, ${safe_copy_data_arg_len:0x}, ${safe_copy_data_arg_interval:0x}, ${safe_copy_data_arg_iterations:0x} ; {dest_offset:0x}, {source_offset:0x}"
						else:
							opcode_output_str = f"safe_copy_data {dest_label}, {source_label}, ${safe_copy_data_arg_len:0x}, ${safe_copy_data_arg_interval:0x}, ${safe_copy_data_arg_iterations:0x}"
						offset += 9
					elif target_label == "Func_58d":
						if fill_arg_2 != 0:
							opcode_output_str = f"fill_mem ${fill_arg_1:0x}, ${fill_arg_2:0x}, ${fill_arg_3:0x}"
						else:
							opcode_output_str = f"fill_mem ${fill_arg_1:0x}, ${fill_arg_3:0x}"
						offset += 4
					elif target_label == "Func_967":
						dest_label = self.find_label(decompress_arg_addr, decompress_arg_bank)
						if dest_label is None:
							dest_offset = get_global_address(decompress_arg_addr, decompress_arg_bank)
							opcode_output_str = f"decompress ${decompress_arg_addr:04x}, ${decompress_arg_bank:02x}, ${decompress_arg_3:0x} ; {dest_offset:0x}"
						else:
							opcode_output_str = f"decompress {dest_label}, ${decompress_arg_3:0x}"
						offset += 4
					elif target_label == "Func_621":
						dest_label = self.find_label(copy_data_arg_dest_addr, copy_data_arg_dest_bank)
						source_label = self.find_label(copy_data_arg_source_addr, copy_data_arg_source_bank)
						if dest_label is None or source_label is None:
							dest_offset = get_global_address(copy_data_arg_dest_addr, copy_data_arg_dest_bank)
							source_offset = get_global_address(copy_data_arg_source_addr, copy_data_arg_source_bank)
							opcode_output_str = f"func_621 ${copy_data_arg_dest_addr:04x}, ${copy_data_arg_dest_bank:02x}, ${copy_data_arg_source_addr:04x}, ${copy_data_arg_source_bank:02x}, ${func_621_arg_6:0x}, ${func_621_arg_len:0x}, ${func_621_arg_interval:0x}, ${func_621_arg_iterations:0x} ; {dest_offset:0x}, {source_offset:0x}"
						else:
							opcode_output_str = f"func_621 {dest_label}, {source_label}, ${func_621_arg_6:0x}, ${func_621_arg_len:0x}, ${func_621_arg_interval:0x}, ${func_621_arg_iterations:0x}"
						offset += 13
...
if __name__ == "__main__":
	...
	ap.add_argument("-s", dest="symfile", default="pokepuzzle.sym")
